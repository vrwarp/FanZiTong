...
- **Drill interleaving.** After every 5th answered card the engine looks for a card seen this session that is in Learning/Relearning or has lapses, rotates the modality (cloze → menu → foil → which word, each kind the card supports in turn) for variety, and never drills the same card twice per session. One word, one drill: a studied dish printed on a slip counts as drilled, nothing on one drill is offered as a neighbour or a cloze option on the next, and a queued word the learner has not met yet is never printed on a slip or offered as an option, so its first sight stays a first sight.
